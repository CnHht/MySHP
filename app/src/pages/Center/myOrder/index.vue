      ...
      <div class="orders">
        <table class="order-item" v-for="(order,index) in orderInfo.records" :key="order.id">
          ...
          <tr>
            <th colspan="5">
                      <span class="ordertitle">{{order.createTime}}订单编号：{{order.outTradeNo}} <span
                          class="pull-right delete"><img :src="order.imgUrl"></span></span>
            </th>
          </tr>
          </thead>
          <tbody>
          <tr v-for="(cart,index) in order.orderDetailList" :key="cart.id">
            ...
            <!--    合并单元格的数量等于 order.orderDetailList.length 并且只展示第一个       -->
...
<!--        'pageNo','pageSize','total','continues'-->
        <MyPagination :pageNo="page" :pageSize="limit" :total="orderInfo.total" :continues="5"  @getPageNo="getPageNo"></MyPagination>
      </div>
    </div>
    <!--猜你喜欢-->
    <div class="like">
...
export default {
  ...
  methods: {
    async getData() {
      ...
      let result = await this.$API.reqCenterOrder(page, limit)
      if (result.code == 200) {
